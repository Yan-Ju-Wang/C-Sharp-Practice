...
<script type="text/javascript" src="http://maps.google.com/maps/api/js?sensor=true&key=開發者GoogleMapApi"></script>
<style>
	#mapDiv {
	width: 90%;
	height: 80%;
      ...
      <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:ConnectionString %>" SelectCommand="SELECT * FROM 美食 WHERE 編號=@編號">
		...
		DataKeyNames="編號" DataSourceID="SqlDataSource1">
             <ItemTemplate>
                 ...
                 <h3>地址：<%# Eval("地址") %></h3>      
                 <a href="#tel:<%# Eval("電話") %>"
					data-role="button" >電話訂位</a>
                 <a href="javascript:GetGeo(<%# Eval("緯度") %>,
				<%# Eval("經度") %>)" data-role="button" >地圖檢視</a>
             </ItemTemplate>
             <LayoutTemplate>
