...
namespace ConsoleGuessNo
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            count = 0;      // 宣告count整數變數用來存放使用者進行猜數字的次數
            min = 0;          // 宣告min整數變數用來存放猜數字的最小值
            max = 100;     // 宣告max整數變數用來存放猜數字的最大值
                           //建立亂數物件r
            Random r = new Random();
            ...
            {
                count += 1;
                Console.Write(" {0}. 猜數子範圍 {1} < ? < {2} ：", count, min, max);
                ...
                if (keyin >= 1 && keyin < 100)   // 判斷keyin是於介於1-99
                {
                    ...
                    Console.WriteLine(" 您已猜了 {0} 次 .... ", count);
                    Console.WriteLine();
                }
                else
                {
